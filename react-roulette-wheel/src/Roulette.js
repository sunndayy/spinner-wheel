...
class Roulette extends React.Component {
  ...
  static defaultProps = {
    ...
    spinTimeTotal: Math.random() * 3000 + 7 * 1000,
  };
  ...
  drawRouletteWheel() {
    ...
    if (canvas.getContext) {
      const outsideRadius = baseSize - 0.125 * baseSize;
      const textRadius = baseSize - 0.4 * baseSize;
      const insideRadius = baseSize - 0.625 * baseSize;
      ...
      ctx.fillStyle = 'blue';
      ctx.beginPath();
      ctx.arc(baseSize, baseSize, 0.375 * baseSize, 0, 2 * Math.PI);
      ctx.fill();
      ctx.stroke();

      ctx.fillStyle = 'white';
      ctx.fillText('Quay', baseSize - 25, baseSize, 50);
    }
  }
    ...
    return (
      <div className="roulette">
        <div className="roulette-container">
          <canvas onClick={this.handleOnClick} ref="canvas" width={baseSize * 2} height={baseSize * 2} className="roulette-canvas">           
          </canvas>
        </div>
      </div>
    );
